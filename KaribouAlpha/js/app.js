angular
    .module("karibouAlphaApp", ["ui.router", "azure-mobile-service.module", "LocalStorageModule", "flow", "ui.bootstrap", "checklist-model" ])
    ...
    .factory("AuthenticationService", ["$http", "$q", "localStorageService", "ngAuthSettings", function ($http, $q, localStorageService, ngAuthSettings) {
        ...
        var _obtainAccessToken = function (externalData) {
            ...
            $http.get(serviceBase + "api/account/ObtainLocalAccessToken", { params: { provider: externalData.provider, externalAccessToken: externalData.externalAccessToken } })
                .success(function (response) {
                    localStorageService.set("authorizationData", { token: response.access_token, tokenExpirationDate: response[".expires"], userName: response.userName, refreshToken: "", useRefreshTokens: false });
                    _authentication.isAuth = true;
    ...
    .factory("AuthenticationInterceptorService", ["$rootScope", "$q", "$injector", "$location", "localStorageService", function ($rootScope,$q, $injector, $location, localStorageService) {
        var authenticationInterceptorServiceFactory = {};
        var _request = function (config) {
            console.log('request');
            $rootScope.loaderact = true;
        ...
        authenticationInterceptorServiceFactory.response= _response;
    ...
    .factory("GetCompanyMessages", ["$http", "localStorageService", "ngAuthSettings", function ($http, localStorageService, ngAuthSettings) {
        var serviceBase = ngAuthSettings.apiServiceBaseUri;
        var authenticationServiceFactory = {};
        var _authentication = {
            isAuth: true,
        ...
        var _GetMessages = function (Id) {

            var model = { CompanyId: Id }
            var response = $http({
                method: "post",
                url: "api/GetCompanyMessage",
                dataType: "json",
                data: JSON.stringify(model),
                headers: { "Content-Type": "application/json" }
            });
        ...
        authenticationServiceFactory.GetMessages = _GetMessages;
        return authenticationServiceFactory;
    }])
...
.directive('fileReader', function () {
                ...
                if (files.length) {
                    ...
                    r.onload = function (e) {
                        ...
                        scope.$apply(function () {
                            scope.$parent.$ctrl1.csvData = contents;
                            scope.$parent.$ctrl1.fileUploadedCompleted = true;
                        });
                    };
    ...
    .directive('tagManager', function (skillListData, TransferData) {
        ...
        return {
            ...
            template:
                '<div class="row">'+
                          '<label class="col-sm-3 account-company-details mandatory ">Core services</label>'+
                        
                            
                '<div class="col-sm-6 dropdown open account-company-details"><input type="text"  id="SelectCoreService" name="SelectCoreService" typeahead-on-select="onSelectSkills($item, $model, $label, $event)" uib-typeahead="skl.SkillName for skl in skills | filter:$viewValue" class="form-control account-company-details"  placeholder="Marketing, App dev, Sales..." ng-model="new_value"></input><span class="alert-danger-new" ng-show=skillNewValueRequired>Please enter core service.</span><span class="alert-danger-new" ng-show=UniqueSkillError>Please enter unique core service.</span> ' +
...
.controller("associateController", ["$scope", "$location", "$timeout", "AuthenticationService", function ($scope, $location, $timeout, AuthenticationService) {
    ...
    .controller("signupController", ["$scope", "$rootScope", "$timeout", "AuthenticationService", "ngAuthSettings", "$state", "Azureservice", "localStorageService", "$location", function ($scope, $rootScope, $timeout, AuthenticationService, ngAuthSettings, $state, Azureservice, localStorageService, $location) {
    ...
    controller.initCheckCode = function () {
        if ($state.params) {
            var code = $state.params.Code;
            if (code != null && code != "") {
                Azureservice.invokeApi("GetInviteByCode?code=" + code, { method: "GET" })
                    .then(function (invite) {
                        $scope.inviteRequest = invite;
                        controller.signupData = {
                            Email: "",
                            Password: "",
                            ConfirmPassword: "",
                        ...
                        controller.signupData.Email = invite.EmailAddress;
                    });
            }
        }
    };
    ...
    $scope.signup = function () {
        ...
        $scope.PasswordNotMatched = false;
        $scope.validateError = false;
        $(".form-control").removeClass("fieldRequiredError");
        $(".registration-email-box").removeClass("fieldRequiredError");
        $(".registration-password").removeClass("fieldRequiredError");
        
        if (!$scope.myForm.$valid) {
            $(".ng-invalid-required").addClass("fieldRequiredError");
        }
        ...
        //    $scope.validateError = true;
        //}
        if ($scope.myForm.$valid && controller.signupData.Password.trim() != controller.signupData.ConfirmPassword.trim()) {
            $scope.Error = false;
            $scope.LengthError = false;
            $scope.PasswordNotMatched = true;
            $scope.validateError = true;
        }
        else if ($scope.myForm.$valid &&(controller.signupData.Password.trim().length < 6 || controller.signupData.ConfirmPassword.trim().length < 6)) {
            $scope.validateError = true;
    ...
    $scope.facebookLogin = function () {
        ...
        var redirectUri = location.protocol + "//" + location.host + "/FaceBookLogin";
        redirectUri = "http://inheritxdev.net:8490/FaceBookLogin";
        //redirectUri = "http://localhost:53009/FaceBookLogin";
        var oauthWindow = window.open(redirectUri, "Authenticate Account", "location=0,status=0,width=600,height=750");
        oauthWindow.$windowScope = controller;
    };

    $scope.googleLogin = function () {
        window.$windowScope = controller;
        var redirectUri = location.protocol + "//" + location.host + "/GoogleLogin";
        var redirectUri = "http://localhost:53009/GoogleLogin";
        redirectUri = "http://inheritxdev.net:8490/GoogleLogin";
        var oauthWindow = window.open(redirectUri, "Authenticate Account", "location=0,status=0,width=600,height=750");
        oauthWindow.$windowScope = controller;
    };

    $scope.linkedInLogin = function (provider) {
        var externalProviderUrl = "http://inheritxdev.net:8490/LinkedInMVC";
    ...
    controller.facebookAuthenticationCompletedCallback = function (fragment) {
        $scope.$apply(function () {
            ...
            if (fragment.hascervituser ==="False" && fragment.haslocalaccount === "False") {
                AuthenticationService.logOut();
            ...
            else if (fragment.hascervituser === "False") {
                AuthenticationService.obtainAccessToken(externalData)
                    .then(function (response) {
                        controller.signupData.NewUser = false;
                        controller.signupData.Email = response.userName;
            ...
            else if (fragment.hascervituser === "True"){
                controller.signupData.NewUser = false;
                controller.signupData.Email = fragment.email;
                $scope.Error = true;
            }
        });
    };

    $scope.authCompletedCBLNK = function (objPassed) {
        $scope.$apply(function () {
            if (objPassed.Result) {
                if (objPassed.haslocalaccount == 'False') {
                    AuthenticationService.logOut();
                    AuthenticationService.externalAuthData = {
                        provider: "LinkedIn",
                        userName: '',
                        externalAccessToken: objPassed.access_token
                    };
                    $location.path('/linkedin');
                }
                else {

                    var externalData = { provider: "LinkedIn", externalAccessToken: objPassed.access_token };
    ...
    controller.registrationKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "email":
                    $("#email").removeClass("fieldRequiredError");
                    break;
                case "password":
                    $("#password").removeClass("fieldRequiredError");
                    break;
                case "ConfirmPassword":
                    $("#ConfirmPassword").removeClass("fieldRequiredError");
                    break;
            }
        }
    }

    controller.initCheckCode();
}])
    .controller("loggedInHeaderController", ["$scope", "$rootScope", "Azureservice", "localStorageService", "$location", function ($scope, $rootScope, Azureservice, localStorageService, $location) {
    var controller = this;
    ...
    controller.redirectToSearchResult = function () {
        if ( controller.searchQuery == undefined || controller.searchQuery == "" ) {
            
        }
        else if (controller.searchQuery.length >=3) {
            $location.path("/search/" + controller.searchQuery + "/companies");
        }
    }
...
.controller("personalProfileDetailsController", ["$scope", "Azureservice", "localStorageService", "$timeout", "$rootScope", "Countries", function ($scope, Azureservice, localStorageService, $timeout, $rootScope, Countries) {
    ...
    controller.updateMyPersonalProfile = function ($flow) {
        $(".form-control").removeClass("fieldRequiredError");
        $("#ddlCountry").removeClass("fieldRequiredError");
        var countrySelected = true;
        if (controller.user.Country == null || controller.user.Country == "") {
            $("#ddlCountry").addClass("fieldRequiredError");
            countrySelected = false;
        }
        if ($scope.myForm.$valid && countrySelected) {
            var authData = localStorageService.get("authorizationData");
    ...
    controller.companyProfileKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "mobile":
                    $("#mobile").removeClass("fieldRequiredError");
                    break;
                case "Email":
                    $("#Email").removeClass("fieldRequiredError");
                    break;
                case "LastName":
                    $("#LastName").removeClass("fieldRequiredError");
                    break;
                case "FirstName":
                    $("#FirstName").removeClass("fieldRequiredError");
                    break;
                case "citytown":
                    $("#citytown").removeClass("fieldRequiredError");
                    break;
            }
        }
    }
...
.controller("companyProfileDetailsController", ["$scope", "Azureservice", "localStorageService", "$timeout", "$rootScope", "TransferData", "Countries", function ($scope, Azureservice, localStorageService, $timeout, $rootScope,TransferData, Countries, CompanySectors) {
    ...
    $scope.tags = [];
    $scope.NoHeaderImage = false;
    
    //$scope.countries = Countries;
    $scope.SelectedCountries =[];
    $scope.urlPattern = /((?:https?\:\/\/|www\.)(?:[-a-z0-9]+\.)*[-a-z0-9]+.*)/i
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("GetMyCompanyProfile", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (company) {
            ...
            //  controller.company.Image = (!controller.company.Image) ? "/img/logo-home-full.png" : "data:image/JPEG;base64," + controller.company.Image
              //  controller.company.HeaderImage = (!controller.company.HeaderImage) ? "/img/user_image.svg" : "data:image/JPEG;base64," + controller.company.HeaderImage;
              if (company.HeaderImage == null || company.HeaderImage== "")
              {
                  $scope.NoHeaderImage = true;
              }
              else {
                  $scope.NoHeaderImage = false;
              }
    ...
    controller.setCompanySector = function (sector) {
        ...
        $("#ddlSector").removeClass("fieldRequiredError");
    };
    ...
    controller.updateCompanyUri = function () {
        ...
        $("#companyUri").removeClass("fieldRequiredError");
    };
    ...
    controller.updateMyCompanyProfile = function ($flow) {
        ...
        $("#ddlSector").removeClass("fieldRequiredError");
        var sectorSelected = true;
        if (controller.company.Sector == null || controller.company.Sector == "") {
            $("#ddlSector").addClass("fieldRequiredError");
        ...
        if ($scope.myForm.$valid && sectorSelected) {
            $rootScope.loaderact = true;
    ...
    controller.getCompanyProfileImage = function () {
        if (controller.company == undefined || !controller.company.Image) {
    ...
    $scope.getCompanyHeaderImage = function () {
        if (controller.company == undefined || !controller.company.HeaderImage) {
            return "/img/user_image.svg";
        }
    ...
    controller.companyProfileKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "TradingName":
                    $("#TradingName").removeClass("fieldRequiredError");
                    break;
                case "phone":
                    $("#phone").removeClass("fieldRequiredError");
                    break;
                case "Email":
                    $("#Email").removeClass("fieldRequiredError");
                    break;
                case "details":
                    $("#details").removeClass("fieldRequiredError");
                    break;
                case "DisplayName":
                    $("#DisplayName").removeClass("fieldRequiredError");
                    break;
                case "companyUri":
                    $("#companyUri").removeClass("fieldRequiredError");
                    break;
    ...
    .controller('ModalImportCSVCtrl', ["$uibModalInstance", "Azureservice", "localStorageService", function ($uibModalInstance, Azureservice, localStorageService) {
        var controller = this;
        controller.ShowError = false;
        controller.ErrorMessage = "";
        controller.csvData = '';
        controller.fileUploadedCompleted = false;

        controller.inviteByImportCSV = function () {
            controller.ShowError = false;
            controller.ErrorMessage = "";
            var authData = localStorageService.get("authorizationData");
            controller.InviteDetail = {};
            controller.InviteDetail.EmailAddress = controller.csvData;
            Azureservice.invokeApi("InviteToRegisterInCompany", { method: "POST", headers: { "Authorization": "Bearer " + authData.token }, body: controller.InviteDetail })
                .then(function (response) {
                    if (response.Success) {
                        controller.InviteUser = "";
                        $uibModalInstance.close(response.SuccessMessage);
                    }
                    else {
                        controller.ShowError = true;
                        controller.ErrorMessage = response.ErrorMessage;
                    }
    ...
    .controller('ModalInstanceCtrl', ["$uibModalInstance", "Azureservice", "localStorageService", function ($uibModalInstance, Azureservice, localStorageService) {
        var controller = this;
        controller.ShowError = false;
        controller.ErrorMessage = "";

        controller.inviteByEmailAddress = function () {
            if (controller.InviteUser != null && controller.InviteUser != "") {
                controller.ShowError = false;
                controller.ErrorMessage = "";
                var authData = localStorageService.get("authorizationData");
                controller.InviteDetail = {};
                controller.InviteDetail.EmailAddress = controller.InviteUser;
                ...
                controller.ShowError = true;
                controller.ErrorMessage = "Email is required.";
            }
        };
    ...
    .controller("companyProfileTeamController", ["$scope", "Azureservice", "localStorageService", "$timeout", "$rootScope", "$uibModal", function ($scope, Azureservice, localStorageService, $timeout, $rootScope, $uibModal) {
    var controller = this;
    ...
    controller.openInviteByEmailPopUp = function (size, parentSelector) {
        ...
        var modalInstance = $uibModal.open({
            ...
            controllerAs: '$ctrl',
            size: size,
            windowTopClass: 'fix_modal_size',
            appendTo: parentElem
            
        });
        modalInstance.result.then(function (selectedItem) {
            controller.showConfirmation(selectedItem);
        }, function () {
            //$log.info('Modal dismissed at: ' + new Date());
        });
    };

    controller.openImportCsvPopUp = function (size, parentSelector) {
        var parentElem = parentSelector ?
            angular.element($document[0].querySelector('.modal-demo ' + parentSelector)) : undefined;
            var modalInstance = $uibModal.open({
                animation: true,
                ariaLabelledBy: 'modal-title',
                ariaDescribedBy: 'modal-body',
                templateUrl: 'myModalImportCSV.html',
                controller: 'ModalImportCSVCtrl',
                controllerAs: '$ctrl1',
                size: size,
                windowTopClass: 'fix_modal_size',
                appendTo: parentElem
    ...
    controller.editCompanyMember = function (companyMember) {
        ...
        companyMember.ProductIdSelected = [];
        companyMember.userLevels = $scope.userLevels;
        controller.showProductListToManage(companyMember);
    };

    controller.showProductListToManage = function (companyMember) {
        var authData = localStorageService.get("authorizationData");
        if (companyMember.UserLevelId > 0) {
            var isAdminLevel = false;
            for (var i = 0; i < $scope.userLevels.length; i++) {
                if ($scope.userLevels[i].Id === companyMember.UserLevelId) {
                    if ($scope.userLevels[i].ShowProductEditOption) {
                        isAdminLevel = true;
                        Azureservice.invokeApi("GetByUserIdPublicCompanyProducts" , { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
                            .then(function (products) {
                                companyMember.ProductsOfUser = products;
                                Azureservice.invokeApi("GetProductSelectedAsTeamMember?userId=" + companyMember.UserID + "&userLevelId=" + companyMember.UserLevelId, { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
                                    .then(function (teamProductIds) {
                                        companyMember.ProductIdSelected = teamProductIds;
                                    });
                            });
                    }
                }
            ...
            if (isAdminLevel) {
                companyMember.ProductIdSelected = [];
                $scope.userlevelClass = true;
            }
            else{
                companyMember.ProductsOfUser = [];
                $scope.userlevelClass = false;
            }
        }
        return false;
    ...
    controller.updateCompanyMember = function (companyMember) {
        ...
        Azureservice.invokeApi("UpdateMemberOfMyCompany", { method: "POST", headers: { "Authorization": "Bearer " + authData.token }, body: companyMember })
            .then(function (responseUpdateCompanyMember) {
                ...
                if (isAdminLevel) {
                    var arrProductIDs = companyMember.ProductIdSelected;
                    var postItems = [];
                    for (var i = 0; i < arrProductIDs.length; i++) {
                            var postItem = new Object();
                            postItem.ProductID = arrProductIDs[i];
                            postItem.UserID = companyMember.UserID;
                            postItem.UserLevelId = companyMember.UserLevelId;
                            postItems.push(postItem);
                    }
                    if (arrProductIDs.length > 0) {
                        Azureservice.invokeApi("UpdateProductTeamMemberMutiple", { method: "POST", headers: { "Authorization": "Bearer " + authData.token }, body: postItems})
                            .then(function (response) {
                                Azureservice.invokeApi("GetMyCompanyMembers", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
                                    .then(function (companyMembers) {
                                        controller.companyMembers = companyMembers;
                        ...
                        var postItem = new Object();
                        postItem.ProductID = 0;
                        postItem.UserID = companyMember.UserID;
                        Azureservice.invokeApi("RemoveProductTeamMemberMapping", { method: "POST", headers: { "Authorization": "Bearer " + authData.token }, body: postItem })
                            .then(function (response) {
                                Azureservice.invokeApi("GetMyCompanyMembers", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
                                    .then(function (companyMembers) {
                                        controller.companyMembers = companyMembers;
        ...
        if (controller.InviteUser != null && controller.InviteUser != "") {
            ...
            Azureservice.invokeApi("InviteToRegisterInCompany", { method: "POST", headers: { "Authorization": "Bearer " + authData.token }, body: controller.InviteDetail })
                .then(function (response) {
                    ...
                    controller.InviteUser = "";
                    if (response == "true") {
                        controller.showConfirmation("Invitation link has been sent.");
                    }
                    if (response == "already sent") {
                        controller.showConfirmation("Invitation link has already sent.");
                    }
...
.controller("productFilesController", ["$scope", "Azureservice", "localStorageService", "$timeout", "$rootScope", "$stateParams", "ProductFileCategories", "ProductFilePrivacyEnum", "$uibModal", function ($scope, Azureservice, localStorageService, $timeout, $rootScope, $stateParams, ProductFileCategories, ProductFilePrivacyEnum, $uibModal) {
    ...
    controller.hasConfirmation = false;
    $scope.PrivacyGroup = [];
    $scope.PublicModel = { ID:0,Name:"NOKIA" };
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("GetProductFilesForEditing?productId=" + controller.productId + "&followerGroupId=" + (controller.selectedFollowerGroup === null ? "null" : controller.selectedFollowerGroup.ID), { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (productFiles) {
              //for (i = 0; i < productFiles.length; i++) {
              //    if (productFiles[i].GroupsVisibleTo.length == 0) {
              //        productFiles[i].Privacy = 0;
              //    }
              //}
    ...
    $scope.SelectedGroup = function (check)
    {
        if ($scope.PrivacyGroup.indexOf(check) == -1) {
            $scope.PrivacyGroup.push(check);
        }
        else {
            $scope.PrivacyGroup.pop(check);
        }
    ...
    //bellow function's perameters temporary removed .
    controller.setProductFilePrivacy = function (productFile, productFilePrivacyGroup) {
        var authData = localStorageService.get("authorizationData");
        controller.busy = true;
        var groupId = productFilePrivacyGroup.ID;
        var privacy = 0;
        if (groupId > 0){
            privacy = 3;
        }
        Azureservice.invokeApi("SetProductFilePrivacy?id=" + productFile.ID + "&privacy=" + privacy +  "&selectedGroupId=0" + groupId, { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
            .then(function () {
                
                //controller.getProductFileGroupVisibilityDisplay(productFile);
...
.controller("productFileUploadController", ["$scope", "Azureservice", "localStorageService", "$rootScope", "ProductFileCategories", "$state", "$stateParams", "$http", function ($scope, Azureservice, localStorageService, $rootScope, ProductFileCategories, $state, $stateParams, $http) {
    ...
    controller.confirmationMessage = null;
    controller.hasConfirmation = false;
    controller.VisibilityOptions = ["Public", "Private"];
    ...
    controller.uploadProductFiles = function () {
        var authData = localStorageService.get("authorizationData");
        var productNotChosse = false;
        var categoryNotChose = false;
        for (var i = 0; i <= controller.productFiles.length - 1; i++){
            var productID = controller.getProductFileProductID(controller.productFiles[i]);
            if (productID == 0) {
                productNotChosse = true;
            }
            var categoryID = controller.getProductFileCategoryID(controller.productFiles[i]);
            if (categoryID == 0) {
                categoryNotChose = true;
            }
        }
        $(".dropdown-toggle").removeClass("notSelectedError");

        if (productNotChosse) {
            $("[productid=0]").parent().parent().find("#chooseProductSelection").addClass("notSelectedError");
        }

        if (categoryNotChose) {
            $("[categoryid=0]").parent().parent().find("#chooseProductCategorySelection").addClass("notSelectedError");
        }

        if (!productNotChosse && !categoryNotChose) {
            controller.busy = true;
    ...
    controller.setProductFileVisibilityOption = function (visibilityOption, productFile) {
        productFile.Visibility = visibilityOption;
    };

    controller.getProductFileVisibility = function (productFile, defaultValue) {
        if (!productFile.Visibility) {
            return defaultValue;
        }
        return productFile.Visibility;
    };
    controller.getProductFileProductID = function (productFile) {
        if (!productFile){
            return 0;
        }
        if (typeof productFile.ProductID == undefined) {
            return 0;
        }
    ...
    controller.getProductFileCategoryID = function (productFile) {
        if (!productFile) {
            return 0;
        }
        if (typeof productFile.Category == undefined) {
            return 0;
        }
        if (!productFile.Category) {
            return 0;
        }
        return productFile.Category;
    };
    ...
    .controller("dashboardController", ["$scope", "Azureservice", "localStorageService", "$stateParams", "GetCompanyMessages", "$state", function ($scope, Azureservice, localStorageService, $stateParams, GetCompanyMessages, $state) {
    var controller = this;

    controller.editing = false;
    controller.showFeedbackEntry = false;
    controller.products = [];
    controller.articles = [];
    ...
    $scope.MessageModel = { LinkUrl: '', Message: '', Image: '', msgerrorr: false, issubmit: false };
    $scope.MessageModel.issubmit = true;
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("GetMyCompany", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (company) {
                controller.company = company;
                var result = GetCompanyMessages.GetMessages(controller.company.ID);
                result.then(function (response) {
                    $scope.CompanyMessages = response.data;
                    $scope.MessageCompanyName = controller.company.DisplayName;;
                })
                if (controller.company.HeaderImage == null || controller.company.HeaderImage == "") {
                    $scope.NoHeaderImage = true;
                }
                else {
                    $scope.NoHeaderImage = false;
    ...
    $scope.getDefaultHeaderImage = function () {
        return "/img/company_new.png";
    };
    ...
    $scope.ShowMessageOption = function () {
        $scope.feedbackVisible = true;
        $scope.MessageVisible = true;
        $scope.ShowMessageButton = false;
    }

    $scope.HideMessageOption = function () {
        $scope.feedbackVisible = false;
        $scope.MessageVisible = false;
        $scope.ShowMessageButton = true;
    }
    ...
    $scope.SendMessageToCompany = function () {
        $scope.MessageModel.CompanyId = controller.company.ID;
        $scope.MessageModel.issubmit = true;
        if ($scope.MessageModel.Message.trim() !== '' || $scope.MessageModel.LinkUrl.trim() !== '' || $scope.MessageModel.Image.trim() !== '') {
            $scope.MessageModel.msgerrorr = false;
            var authData = localStorageService.get("authorizationData");
            Azureservice.invokeApi("MessageToCompany", { method: "POST", headers: { "Authorization": "Bearer " + authData.token }, body: $scope.MessageModel })
                .then(function (Response) {
                    $scope.MessageModel = { LinkUrl: '', Message: '', Image: '' };
                    var result = GetCompanyMessages.GetMessages(controller.company.ID);
                    result.then(function (response) {
                        $scope.CompanyMessages = response.data;
                        $scope.MessageCompanyName = controller.company.DisplayName;
            ...
            $scope.MessageModel.msgerrorr = true;
        }
    }

    controller.GetSubbmitedImage = function () {
        if (!$scope.MessageModel.Image) {
            return "http://placehold.it/212x212";
        }
        return "data:image/JPEG;base64," + $scope.MessageModel.Image;
    };

    controller.setMessageLinkUrl = function () {
        $scope.MessageModel.LinkUrl;
        controller.OpenMessagePopupLinkUrl = false;
    };

    $scope.AddMessageLinkUrl = function () {
        controller.OpenMessagePopupLinkUrl = !controller.OpenMessagePopupLinkUrl;
    };

    controller.MessageDisplayDate = function (MessageValue) {
        if (!MessageValue.Date) {
            return null;
        }
        var date = moment(MessageValue.Date);
    ...
    controller.MessageImageSubmitted = function ($files, $event, $flow) {
        ...
        reader.addEventListener("load", function () {
            ...
            base64DataBeginIndex = reader.result.indexOf(base64DataBeginMarker);
            $scope.MessageModel.Image = reader.result.substring(base64DataBeginIndex + base64DataBeginMarker.length);
            $flow.cancel();
            $scope.$apply();
        }, false);
        reader.readAsDataURL($files[0].file);
    ...
    controller.showFeedbackEntrySection = function(varId)   {
        $(".feedback" + varId).toggle();
       
    }
    controller.GoToEditCompany = function () {
...
.controller("myCompanyController", ["$rootScope", "$scope", "Azureservice", "localStorageService", "$stateParams", "$state", "GetCompanyMessages", function ($rootScope,$scope, Azureservice, localStorageService, $stateParams, $state, GetCompanyMessages) {
    ...
    $scope.getCompanyProfileImage = function () {
        if (!controller.company.Image) {
            return "/img/user_image.svg";
        }
        return "data:image/JPEG;base64," + controller.company.Image;
    ...
    controller.CloseMessageLinkUrl = function ()
    {
    ...
    controller.getFeedbackDisplayDate = function (MessageValue) {
        if (!MessageValue.Date) {
            return null;
        }
...
.controller("companyController", ["$scope", "Azureservice", "localStorageService", "$stateParams", "$state", function ($scope, Azureservice, localStorageService, $stateParams, $state) {
    var controller = this;
    ...
    $scope.isViewerLevel = false;
    $scope.isEditorLevel = false;
    controller.companyConnections = [];
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("GetCompanyMemberLevelByCompanyId?companyId=" + controller.companyId, { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (companyMemberInfo) {
              controller.levelInfo = companyMemberInfo;
              if (companyMemberInfo.UserLevel == "Admin")
                  $scope.isAdminLevel = true;
              if (companyMemberInfo.UserLevel == "Viewer")
                  $scope.isViewerLevel = true;
              if (companyMemberInfo.UserLevel == "Editor")
                  $scope.isEditorLevel = true;
          });


        Azureservice.invokeApi("GetProductSharedByCompany?companyId=" +  controller.companyId, { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (products) {
              controller.products = products;
              $scope.$broadcast("dataready", null);
          });
    ...
    $scope.GoToEditCompany = function (companyId) {
        console.log("go to company " + companyId);
        //this is wrong need to correct
        $state.go("companyProfileDetails");
    }
...
.controller("myProductController", ["$scope", "Azureservice", "localStorageService", "$stateParams", "$http", function ($scope, Azureservice, localStorageService, $stateParams, $http, GetCompanyMessages) {
    var controller = this;
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("CheckMyProductFileIsPublic?productId=" + controller.productId, { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (resonose) {
              $scope.HideDropDown = resonose;
          });
        ...
        Azureservice.invokeApi("GetProductFilesForEditing?productId=" + controller.productId + "&followerGroupId=" + (controller.selectedFollowerGroup === null ? "null" : controller.selectedFollowerGroup.ID), { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (productFiles) {
              for (var i = 0; i < productFiles.length; i++) {
                  if (productFiles[i].GroupsVisibleTo.length > 0)
                  {
                      for (var j = 0; j < productFiles[i].GroupsVisibleTo.length; j++)
                      {
                          $scope.GroupName.push(productFiles[i].GroupsVisibleTo[j].Name);
                      }
                  }
              }
            });

        Azureservice.invokeApi("GetMyProductAccessLevel?id=" + controller.productId, { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (accessLevel) {
                controller.productAccessLevel = accessLevel;
            });
    };
    ...
    controller.selectFollowerGroup = function (followerGroup) {
        ...
        if (followerGroup == "Public")
        {
            var obj = {ID:0,Name:"Public"};
            
            controller.selectedFollowerGroup = obj;
        }
...
.controller("onboardingStepOneController", ["$scope", "Azureservice", "localStorageService", "$timeout", "$rootScope", "$state", "SectorService", "Countries", "TransferData", function ($scope, Azureservice, localStorageService, $timeout, $rootScope, $state, SectorService, Countries, TransferData, InviteExistingUser) {
    ...
    $scope.skillList = {};
    $scope.dataHasLoaded = false;
    $scope.hasCompanyLogo = false;
    $scope.hasProfileLogo = false;
    ...
    controller.nextStep = function ($flow) {
        ...
        if (!$scope.myForm.$valid && controller.companySelectionStatus.CompanySelectionType == 2) {
            $(".ng-invalid-required").addClass("fieldRequiredError");
        }
        if (!$scope.PersonalProfile.$valid && controller.companySelectionStatus.CompanySelectionType == 3) {
            $(".ng-invalid-required").addClass("fieldRequiredError");
        }
        ...
        else if ($scope.PersonalProfile.$valid && controller.companySelectionStatus.CompanySelectionType == 3) {
    ...
    controller.getCompanyProfileImage = function () {
        if (!controller.company.Image) {
            return "/img/cervitt-avatar.png";
        }
        return "data:image/JPEG;base64," + controller.company.Image;
    };

    controller.companySelectionChanged = function () {
        $(".form-control").removeClass("fieldRequiredError");
    }
    ...
    $scope.ngSectorOptionSelected = function (value) {
        if (arguments.length) {
            ...
            $("#SelectSector").removeClass("fieldRequiredError");
        } else {
    ...
    controller.flowFilesSubmittedUser = function ($files, $event, $flow) {
        if (!$event) {
            return;
        }
    ...
    controller.addCompanyKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "Phone":
                    $("#Phone").removeClass("fieldRequiredError");
                ...
                case "Email":
                    ...
                    break;
                case "description":
                    $("#description").removeClass("fieldRequiredError");
                    break;
                case "trading":
                    $("#trading").removeClass("fieldRequiredError");
                    break;
                case "company":
                    $("#company").removeClass("fieldRequiredError");
                    break;
            }
        }
    };

    controller.addPersonalProfileStepOneKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "Town":
                    $("#pfTown").removeClass("fieldRequiredError");
                    break;
                case "PhoneNumber":
                    $("#pfPhoneNumber").removeClass("fieldRequiredError");
                    break;
                case "Surname":
                    $("#pfSurename").removeClass("fieldRequiredError");
                    break;
                case "FirstName":
                    $("#pfFirstName").removeClass("fieldRequiredError");
                    break;
            }
        }
    }
...
.controller("onboardingStepTwoController", ["$scope", "Azureservice", "localStorageService", "$timeout", "$rootScope", "$state", function ($scope, Azureservice, localStorageService, $timeout, $rootScope, $state) {
    ...
    controller.nextStep = function () {
        ...
        if (!controller.product.ID && $scope.myForm.$valid) {
    ...
    controller.jumpToStep = function (stepName) {
        var authData = localStorageService.get("authorizationData");
        controller.busyBack = true;
        controller.resetNotifications();
        ...
        Azureservice.invokeApi("JumpToWizardStep", { method: "POST", body: controller.statusDto, headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (onboardingStatus) {
                if (controller.statusDto.OnboardingStep == 1) {
                    $state.go("onboardingStepOne");
                }
            })
            .finally(function () {
    ...
    controller.addProductKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "Description":
                    $("#Description").removeClass("fieldRequiredError");
                    break;
                case "productName":
                    $("#productName").removeClass("fieldRequiredError");
                    break;
            }
        }
    }
...
.controller("onboardingStepThreeController", ["$scope", "Azureservice", "localStorageService", "$rootScope", "ProductFileCategories", "$state", "$stateParams", "$http", "Countries","$timeout", function ($scope, Azureservice, localStorageService, $rootScope, ProductFileCategories, $state, $stateParams, $http,Countries,$timeout) {
    ...
    controller.nextStep = function () {
        ...
        $(".form-control").removeClass("fieldRequiredError");
        if (!$scope.PersonalDetails.$valid) {
            $(".ng-invalid-required").addClass("fieldRequiredError");
        }
        if ($scope.PersonalDetails.$valid) {
            controller.busy = true;
    ...
    controller.jumpToStep = function (stepName) {
        ...
        Azureservice.invokeApi("JumpToWizardStep", { method: "POST", body: controller.statusDto, headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (onboardingStatus) {
                ...
                if (controller.statusDto.OnboardingStep == 2) {
                    $state.go("onboardingStepTwo");
                }
            })
            .finally(function () {
    ...
    controller.addPersonalProfileKeyPressEvent = function ($event, obj) {
        if (!obj.isEmpty) {
            switch (obj.$name) {
                case "Town":
                    $("#Town").removeClass("fieldRequiredError");
                    break;
                case "PhoneNumber":
                    $("#PhoneNumber").removeClass("fieldRequiredError");
                    break;
                case "Surname":
                    $("#Surname").removeClass("fieldRequiredError");
                    break;
                case "FirstName":
                    $("#FirstName").removeClass("fieldRequiredError");
                    break;
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("GetMyOnboardingStatus", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (onboardingStatus) {
              ...
              $scope.IsIndividual = onboardingStatus.IsIndividual;
              if (!onboardingStatus.OnboardingStep) {
                  return;
              }
              if (onboardingStatus.OnboardingStep === 1) {
    ...
    controller.jumpToStep = function (stepName) {
        ...
        Azureservice.invokeApi("JumpToWizardStep", { method: "POST", body: controller.statusDto, headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (onboardingStatus) {
                ...
                if (controller.statusDto.OnboardingStep == 3) {
                    $state.go("onboardingStepThree");
                }
            })
            .finally(function () {
...
.controller("onboardingCompletedController", ["$scope", "Azureservice", "localStorageService", "$rootScope", "ProductFileCategories", "$state", "$stateParams", "$http", "Countries", "$timeout", function ($scope, Azureservice, localStorageService, $rootScope, ProductFileCategories, $state, $stateParams, $http, Countries, $timeout) {
    var controller = this;
    $scope.companyUrl = "";
    $scope.productUrl = "";
    $scope.productNameLive = "";
    ...
    controller.init = function () {
        ...
        Azureservice.invokeApi("GetMyCompany", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
         .then(function (company) {
             controller.company = company;
             $scope.companyUrl = company.URI;
            });

        Azureservice.invokeApi("GetMyPersonalProfile", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (user) {
                $scope.individualFlag = user.IsIndividual;
            });

        Azureservice.invokeApi("GetMyCompanyProducts", { method: "GET", headers: { "Authorization": "Bearer " + authData.token } })
          .then(function (products) {
              if(products != null && products.length > 0){
                  $scope.productNameLive = products[0].Name;
                  $scope.productUrl = products[0].URI;
              }
          });
    }
    ...
    controller.jumpToStep = function (stepName) {
        ...
        Azureservice.invokeApi("JumpToWizardStep", { method: "POST", body: controller.statusDto, headers: { "Authorization": "Bearer " + authData.token } })
            .then(function (onboardingStatus) {
                ...
                if (controller.statusDto.OnboardingStep == 4) {
                    $state.go("onboardingStepFour");
                }
            })
            .finally(function () {
    ...
    .controller("linkedInController", ["$scope", "$location", "$timeout", "AuthenticationService", "localStorageService", "$state", "Azureservice", function ($scope, $location, $timeout, AuthenticationService, localStorageService, $state, Azureservice) {
        var controller = this;
        $scope.savedSuccessfully = false;
        $scope.message = "";
        ...
        $scope.registerExternal = function () {
            ...
            AuthenticationService.registerExternal($scope.registerData)
                .then(function (response) {
                    AuthenticationService.obtainAccessToken($scope.registerData)
                        .then(function (response) {
                            $scope.savedSuccessfully = true;
                            $scope.message = "User has been registered successfully, you will be redicted to orders page in 2 seconds.";
                            $scope.startTimer();
...
.run(["$rootScope", "$state", "localStorageService", "$timeout", function ($rootScope, $state, localStorageService, $timeout) {
    ...
    $rootScope.$on("$stateChangeSuccess", function (event, toState, toStateParams, fromState, formStateParams) {
        $rootScope.cssname = 'dist_dev';
        if (toState.url == "/pricing" || toState.url == "/" || toState.url == "/demo" || toState.url == "/terms" || toState.url == "/policy" || toState.url == "/privacy" || toState.url == "/security") {
            $rootScope.cssname = 'dist';
        }

        $rootScope.isSocial = (toState.name == "myCompany" || toState.name == "myProduct") ? true : false;
        if (document.getElementById('at-expanding-share-button') != undefined) {
            if (toState.name == "myCompany" || toState.name == "companyProfileNews") {
                document.getElementById('at-expanding-share-button').style.visibility = ''
            }
            else {
                document.getElementById('at-expanding-share-button').style.visibility = 'hidden'
            }
        }
